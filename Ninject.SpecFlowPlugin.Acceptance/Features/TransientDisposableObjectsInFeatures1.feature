Feature: Transient disposable feature objects disposed after feature 1
...
to be executed, we need write this feature twice and write a hacky scenario.
    ...
    @ninject
    Scenario: Transient disposable feature dependencies 2 are disposed after feature
	
    Transient disposable feature dependencies are disposed after feature execution
    when we use Ninject.

        Given I have injected TransientDisposableFeatureDependency1 in the binding class StepClassDisposableAfterFeature
        Then TransientDisposableFeatureDependency2 has been disposed if the previous feature had to dispose it
	    
    @bodi
    Scenario: Transient disposable feature dependencies 2 are not disposed after feature
