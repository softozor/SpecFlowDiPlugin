...
so that their resources are freed.
    ...
    @bodi
    Scenario Outline: Transient disposable scenario dependencies are not disposed after scenario
    ...
    when we use BoDi.
        ...
        Given I have injected TransientDisposableScenarioDependency<injected dependency> in the binding class StepClassDisposableAfterScenario
        Then TransientDisposableScenarioDependency<disposed dependency> has not been disposed if the previous scenario had to dispose it
